...
var nLessons = 6;
var lessonStatus = "002200";
var menuURL = "../c01_menu.html";
var bookmark = "";
var courseStatus = "";
var strPagesViewed = ""; 	//This will not be used to simplify code
var intendedClose = true;  	//page unload is intentional or not
var lessonCompletion = false;
...
function toMenu() {
	...
	if (blnLastPage) {
		...
		if (getLesson()< nLessons-2){
			var strLocation = "M1L"+ getLesson() + "/" + getPage() + ".html";
		//alert(strLocation)
...
function getLesson() {
	...
	} else if (strTemp.indexOf("T")>=0) {
		if(getPage().substring(0,3)=="pre") return 5;
		else return 6;
	}
}
...
function saveBookmark() {
  if ( inLMS ) {
	...
	if ( !blnLastPage && ( getPage().indexOf("menu") < 0) && (getLesson() <= nLessons-2) ) {//getLesson() <= nLessons-2 means that pre and post test won't be bookmarked.
